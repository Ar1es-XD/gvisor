	...
	case unix.S_IFCHR:
		if !impl.config.CharDevicePolicy.AllowsPassthrough() {
			logRejectedCharDevOpenOnce.Do(func() {
				log.Warningf("Rejecting attempt to open character device from host filesystem: %q. If you want t    o allow this, set flag --character-device-policy", fd.ControlFD.Node().FilePath())
			})
			return nil, -1, unix.EPERM
		}
	}
