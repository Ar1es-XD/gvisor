...
get_next_ar_version() {
  ...
  # Method 1: Query AR via gcloud CLI
  ...
  # Method 2: Fallback query via PEP 503 Simple Index HTTP API
  if [[ -z "${versions}" ]]; then
    echo "gcloud query returned empty or failed; falling back to AR Simple Index HTTP API..." >&2
    local token
    token="$(gcloud auth print-access-token 2>/dev/null || true)"
    if [[ -n "${token}" ]]; then
      echo "Querying PEP 503 Simple Index at https://us-python.pkg.dev/oss-exit-gate-prod/gvisor--pypi/simple/gvisor/..." >&2
      versions=$(curl -s -f -H "Authorization: Bearer ${token}" \
        https://us-python.pkg.dev/oss-exit-gate-prod/gvisor--pypi/simple/gvisor/ 2>/dev/null \
        | grep -oE "gvisor-${today}\.[0-9]+" | sed "s/^gvisor-//" | sort -u || true)
    fi
  fi
...
build_wheel_sdist() {
  ...
  cp -fv dist/*.whl "${abs_dest_dir}/"
  cp -fv dist/*.tar.gz "${abs_dest_dir}/"
...
upload_to_ar() {
  ...
  echo "Obtaining GCP OAuth2 access token via gcloud auth print-access-token..."
  local token
  token="$(gcloud auth print-access-token)"
  echo "Token successfully retrieved (length: ${#token} characters)."

  echo "Publishing artifacts to https://us-python.pkg.dev/oss-exit-gate-prod/gvisor--pypi..."
  if command -v uv >/dev/null 2>&1; then
    echo "Using uv publish..."
    uv publish \
      --publish-url https://us-python.pkg.dev/oss-exit-gate-prod/gvisor--pypi \
    ...
    python3 -m twine upload \
      ...
      -p "${token}" \
      "${abs_target_dir}"/*.whl "${abs_target_dir}"/*.tar.gz
  elif docker ps --format '{{.Names}}' 2>/dev/null | grep -q 'gvisor-bazel'; then
    local container_name
    container_name="$(docker ps --format '{{.Names}}' | grep 'gvisor-bazel' | head -n 1)"
    echo "Running twine upload inside active Docker container (${container_name})..."
    docker exec "${container_name}" twine upload \
      --repository-url https://us-python.pkg.dev/oss-exit-gate-prod/gvisor--pypi \
      -u oauth2accesstoken \
      -p "${token}" \
      "${abs_target_dir}"/*.whl "${abs_target_dir}"/*.tar.gz
  else
    echo "ERROR: Neither uv nor twine found to publish Python artifacts." >&2
    return 1
  fi
